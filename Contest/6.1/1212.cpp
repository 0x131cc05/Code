...
int coef[11][11][N], debug;

namespace sol2 {

int C[11][11], mx[N], ct[N], hh[N], mn[N];

vector<int> wkr[31], son[N]; int layer;
...
inline int solve(int layer) {
    ...
    tmp += posR - posL + 1;
    int res = 0;
    if (debug) cerr << posL << ' ' << posR << ' ' << tmp << endl;
...
int main() {
    freopen("ex_tree3.in", "r", stdin), freopen("out.out", "w", stdout);
