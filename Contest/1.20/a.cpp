...
vector<int> merge(vector<int> A, vector<int> B, int m) {
    int base = Pow(2, m);
    for (int i = 0, pw = 1; i < A.size(); i++, pw = (LL)pw * base % mod)
        A[i] = (LL)A[i] * pw % mod;
    return Mul(A, B);
}
...
vector<int> solve(int n) {
    ...
    tmp = merge(tmp, tmp, mid);
    if (n & 1) tmp = merge(tmp, dp1, 1);
    return tmp;
}

int main() {
    freopen("a.in", "r", stdin), freopen("a.out", "w", stdout);
    LL n; scanf("%lld%d", &n, &k), frac[0] = inv[0] = 1, dp1.push_back(0);
    if (n > k) return puts("0"), 0;
    for (int i = 1; i <= k; i++) frac[i] = (LL)frac[i - 1] * i % mod, inv[i] = Pow(frac[i], mod - 2), dp1.push_back(inv[i]);
    init(k);
    vector<int> res = solve(n); int ans = 0;
    for (int i = 0; i <= k; i++) ans = (ans + (LL)res[i] * frac[i] % mod * C(k, i)) % mod;
    printf("%d\n", ans);
}
