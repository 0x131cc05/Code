...
tcurts dfs(int len, int pre, int st) {
    ...
    vis[st][len][pre] = cur, dp[st][len][pre] = { 0ll, 0ll };
    for (int i = st; i < 10; i++) 
        dp[st][len][pre] += dfs(len - 1, pre + (i == cur - 1), 0);
    cout << st << ' ' << len << ' ' << pre << ' ' << dp[st][len][pre].mn << endl;
    return dp[st][len][pre];
}

string work(int digit) {
    cur = digit + 1; 
    tcurts pre = { 0ll, 0ll }; int len = 1;
    for (len = 1; len; len++) {
        auto res = dfs(len, 0, 1);
        if ((pre + res).mn < 0ll) break;
        pre += res;
    }
    cout << len << endl;
    string ans;
