...
typedef tuple<BigInt, BigInt, BigInt> P;

int main() {
    int k, n; cin >> k >> n;
    queue<P> q; q.push(P(k, 1, 0)); set<BigInt> s; 
    BigInt zero;
    while (!q.empty()) {
        auto tmp = q.front(); q.pop();
        auto A = get<0>(tmp), B = get<1>(tmp), C = get<2>(tmp);
        if (A != zero && B != zero && C != zero && !s.count(A) && !s.count(B) && !s.count(C)
            && !(A == B || B == C || A == C)) {
            cout << A << ' ' << B << ' ' << C << endl;
            s.insert(A), s.insert(B), s.insert(C);
            if (!--n) return 0;
        } 
        auto push = [&](BigInt A, BigInt B, BigInt C) {
