...
int main() {
    LL R; int B, len = 1; cin >> R >> B, frac[0] = inv[0] = 1;
    for (int i = 1; i <= B + 2; i++) frac[i] = (LL)frac[i - 1] * i % mod;
    inv[B + 2] = Pow(frac[B + 2], mod - 2);
    for (int i = B + 1; i >= 1; i--) inv[i] = (LL)inv[i + 1] * (i + 1) % mod;
    while (len <= 2 * (B + 2)) len <<= 1;
    Pre(len);
    vector<int> X(B + 2), Y(B + 2); int tmp = (R / B + 1) % mod;
    for (int i = 0, t = tmp; i <= B + 1; i++, t = (LL)t * tmp % mod) {
        Y[i] = inv[i + 1];
        X[i] = (LL)(t - 1) * inv[i + 1] % mod;
    }
    Y = Inv(Y, B + 2);
    X = mul(X, Y);
    for (int i = 0; i <= B + 1; i++)
        pw[i] = (LL)X[i] * frac[i] % mod;
    int c = 1, res = 0; R = R % mod;
    for (int i = 0; i <= B; i++) {
        if (i) c = c * (R + B + 1 - i + 1) % mod * inv[i] % mod * frac[i - 1] % mod; 
        int w = ((R + 1) * pw[B - i] - (LL)B * pw[B - i + 1]) % mod;
        res = (res + (LL)c * w) % mod;
    }
    res = (res + mod) % mod;
    printf("%d\n", res);
